...
results = np.full((reps, len(drift_params), n_chunks, 22), np.nan)
pbar = tqdm(total=reps*3*500)
...
for dp_id, dp in enumerate(drift_params):
    for _rs_id, _rs in enumerate(rs):
        ...
        for i in range(n_chunks):
            ...
            cc.fit(X,y)
            # print(cc.complexity)
            # print(len(cc.complexity))
            # exit()
            
            meta_all.append(cc.complexity)
            pbar.update(1)    
