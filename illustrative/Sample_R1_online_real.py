...
data = []
stream = ARFFParser('insects/%s' % files[1], chunk_size=chunk_size, n_chunks=10000000)
while(1):
    try:
...
ax[2].scatter(pca_rep[:,0], pca_rep[:,1], s=10)
...
plt.tight_layout()
plt.savefig('foo.png')
plt.savefig('fig_r1/online-sample_real.png')
plt.savefig('fig_r1/online-sample_real.pdf')
